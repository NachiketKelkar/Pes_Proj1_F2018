...
int invert()
{
   ...
   else
   {
	 ...
	 scanf("%d",&offset);
	 u_inv_add=mem+offset;					//Calculating the starting address of the memory that user wants to display using offset	
	 break;
   ...
   for(count=mem;count<=memlast;count++)			//Loop starts from the starting address of the first block of allocated memory to the starting add of the last block of allocated memory
   {
